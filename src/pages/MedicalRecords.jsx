...
import { Search, Loader2, FileText, Calendar, ShieldAlert } from 'lucide-react';
...
export default function MedicalRecords() {
  ...
  const loadMedicalRecords = async () => {
    try {
      setLoading(true);
      // Mengambil data rekam medis dan melakukan join ke tabel patients untuk mengambil nama & jenis hewan
      const { data, error } = await supabase
        .from('medical_histories') // Jika nama tabel di database kamu berbeda (misal: medical_history), sesuaikan di sini
        .select(`
          id,
          diagnosis,
          treatment,
  ...
  // Fungsi pencarian berdasarkan Nama Hewan, Jenis Hewan, atau isi Diagnosis
  ...
  // Label pemetaan jenis hewan (sama seperti di halaman Patients)
  const speciesLabels = {
    anjing: "Anjing",
    kucing: "Kucing",
    kelinci: "Kelinci",
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 font-barlow">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kumpulan Rekam Medis</h1>
          <p className="text-sm text-gray-500">
            Menampilkan seluruh riwayat diagnosa dan tindakan pasien ({filteredRecords.length} riwayat).
          </p>
        </div>
      ...
      {/* Konten Utama (Status Loading / Kosong / Grid Cards) */}
      {loading ? (
        <div className="flex flex-col items-center justify-center p-20 text-gray-400 gap-2">
          <Loader2 className="animate-spin text-indigo-600" size={32} />
          <span className="text-sm">Sedang mengambil riwayat rekam medis...</span>
        </div>
      ) : filteredRecords.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-2xl p-16 text-center text-gray-400 shadow-sm">
          <div className="flex flex-col items-center gap-2">
            <FileText size={48} className="text-gray-300" />
            <p className="font-medium">{search ? "Hasil pencarian tidak ditemukan." : "Belum ada riwayat rekam medis terdaftar."}</p>
          </div>
        </div>
      ) : (
        // Opsi B: Desain Grid Layout berbentuk Kartu (Card)
            ...
            >
              <div>
                ...
                <div className="flex justify-between items-start gap-4 pb-4 border-b border-gray-50">
                  <div>
                    ...
                    <span className="inline-block mt-1 bg-blue-50 text-blue-700 px-2 py-0.5 rounded text-xs font-medium">
                      {speciesLabels[record.patients?.species] || record.patients?.species || "-"}
                    </span>
                  </div>
                  <div className="flex items-center text-xs text-gray-400 gap-1.5 whitespace-nowrap bg-gray-50 px-2.5 py-1 rounded-lg">
                    <Calendar size={14} />
                    {record.created_at ? new Date(record.created_at).toLocaleDateString("id-ID", { day: 'numeric', month: 'short', year: 'numeric' }) : "-"}
                  </div>
                </div>
                ...
                <div className="space-y-4 my-4">
                  <div>
                    <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Diagnosis / Keluhan</h4>
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Tindakan / Pengobatan (Treatment)</h4>
              ...
              {/* Bagian Bawah Kartu: ID Referensi Opsional */}
              <div className="pt-3 border-t border-gray-50 flex items-center justify-between text-[10px] text-gray-400 font-mono">
                <span>PATIENT ID: {record.patient_id?.substring(0, 8)}...</span>
                <span>APPOINTMENT: {record.appointment_id?.substring(0, 8)}...</span>
              </div>
            </div>
          ))}
        </div>
